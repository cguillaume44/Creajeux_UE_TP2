...
void ASimon_Manager::PlaySequence()
{
	...
	if (CurrentSeq.IsValidIndex(0) && CurrentSeq.IsValidIndex(PreviewSequenceIndex))
	{
		...
		if (Button)
		{
			...
			//call light down function inside Simon_Button on the button after 0.5 sec
			GetWorldTimerManager().SetTimer(LightDownTimer, [Button]() { Button->LightDown(); }, 0.5f, false);

			//call play sequence again after 1.5 sec
			FTimerHandle PlaySequenceTimer;
			GetWorldTimerManager().SetTimer(PlaySequenceTimer, this, &ASimon_Manager::PlaySequence, 1.5f, false);
		}
		else
		{
	...
	if (GetSequence(MainSequenceIndex).IsValidIndex(GuessIndex))
	{
		...
		if (Button && Button == LastButtonPressed)
		{
			...
			if (GuessIndex > GetSequence(MainSequenceIndex).Num()-1)
			{
				...
				{
					//wait 1 sec before triggering the win process
					FTimerHandle WinTimer;
					GetWorldTimerManager().SetTimer(WinTimer, this, &ASimon_Manager::TriggerWinProcess, 1.f, false);
				}
			}
